	...
	public function authenticate($code = null, $complete = false) {
		if ($complete) {
			...
			if($this->config['redirect_uri'] == '') {
				return 'https://instagram.com/oauth/authorize/?client_id=' . $this->config['client_id'] . '&response_type=code';
			} else {
 ...
 */
	public function read(Model $model, $queryData = array(), $recursive = null) {
		...
		case 'Media':
			if (empty($queryData['conditions'])) {
				...
				if (!empty($conditions['id'])) {
					$id = $conditions['id'];
					unset($conditions['id']);
					$returnedData = $this->_request('GET', 'media/' . $id, $conditions);
					$data = array('data' => array(array($returnedData['data'])));
				} elseif (!empty($conditions['url'])) {
					$data = array(
						'data'=> array($this->_request('GET', 'oembed', $conditions))
					);
		...
		case 'InstagramUser':
			$conditions = $this->_extractFields($queryData['conditions'], 'User');
			if (!empty($conditions['id'])) {
				$id = $conditions['id'];
				unset($conditions['id']);

				// If there's only one result, add it at index zero
				$request_data = $this->_request('GET', 'users/' . $id, $conditions);
				$data = array(
				...
				$conditions['q'] = $username;

				$request_data = $this->_request('GET', 'users/search',  $conditions);
				$data = array(
		...
		if(count($data) == 1) {
			// debug($data);
			if(isset($data[0]['Media'])) {
				if(isset($data[0]['Media'][0])) {
					$data[0]['Media'] = $data[0]['Media'][0];
				} else {
					$data[0]['Media'] = $data[0]['Media'];
				}
			}
